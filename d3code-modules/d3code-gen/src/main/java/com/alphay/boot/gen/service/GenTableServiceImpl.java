package com.alphay.boot.gen.service;
...
import com.alphay.boot.gen.mapper.GenTableColumnMapper;
import com.alphay.boot.gen.mapper.GenTableMapper;
import com.alphay.boot.gen.util.GenUtils;
import com.alphay.boot.gen.util.VelocityInitializer;
import com.alphay.boot.gen.util.VelocityUtils;
...
public class GenTableServiceImpl implements IGenTableService {
  ...
  @Resource private GenTableMapper baseMapper;
  ...
  @Override
  public PageResult<GenTable> queryPageList(GenTableQueryParam queryParam) {
    return baseMapper.selectPage(queryParam, buildGenTableQueryWrapper(queryParam));
  }

  private QueryWrapper<GenTable> buildGenTableQueryWrapper(GenTableQueryParam param) {
    QueryWrapper<GenTable> wrapper =
        new QueryWrapperX<GenTable>()
            .eqIfPresent("data_name", param.getDataName())
            .likeIfPresent("lower(table_name)", StringUtils.lowerCase(param.getTableName()))
            .likeIfPresent("lower(table_comment)", StringUtils.lowerCase(param.getTableComment()))
            .betweenIfPresent("create_time", param.getCreateTime())
  ...
  public PageResult<GenTable> queryPageDbTableList(GenTableQueryParam param) {
    ...
    List<String> tableNames = baseMapper.selectTableNameList(param.getDataName());
    String[] tableArrays;
    ...
    List<GenTable> tables =
        tablesMap.values().stream()
            ...
            .toList();
    return PageResult.build(tables, param);
  }
  ...
  public void deleteGenTableByIds(Long[] tableIds) {
    ...
    genTableColumnMapper.delete(
        new LambdaQueryWrapper<GenTableColumn>().in(GenTableColumn::getTableId, ids));
  }
      ...
      for (GenTable table : tableList) {
        ...
        if (row > 0) {
          ...
          List<GenTableColumn> genTableColumns =
              SpringUtils.getAopProxy(this).selectDbTableColumnsByName(tableName, dataName);
  ...
  public void generatorCode(Long tableId) {
    ...
    for (String template : templates) {
      if (!StringUtils.containsAny(
          template, "sql.vm", "api.ts.vm", "types.ts.vm", "index.vue.vm", "index-tree.vue.vm")) {
        // 渲染模板
        StringWriter sw = new StringWriter();
        Template tpl = Velocity.getTemplate(template, Constants.UTF8);
        tpl.merge(context, sw);
  ...
  public void synchDb(Long tableId) {
    ...
    if (CollUtil.isNotEmpty(saveColumns)) {
      genTableColumnMapper.insertOrUpdate(saveColumns);
    }
    List<GenTableColumn> delColumns =
        StreamUtils.filter(
            tableColumns, column -> !dbTableColumnNames.contains(column.getColumnName()));

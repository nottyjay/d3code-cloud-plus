  ...
  <div>
    <el-dialog v-model="roleDialog.visible.value" :title="roleDialog.title.value" width="80%" append-to-body>
      ...
      <el-card shadow="hover">
        ...
        <pagination v-if="total > 0" v-model:total="total" v-model:page="queryParams.pageNum" v-model:limit="queryParams.pageSize" @pagination="pageList" />
      </el-card>
      <template #footer>
...
const getList = () => {
  loading.value = true
  api.listRole(queryParams.value).then((res) => {

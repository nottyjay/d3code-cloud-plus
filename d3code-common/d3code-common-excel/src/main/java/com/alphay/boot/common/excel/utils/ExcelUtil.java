package com.alphay.boot.common.excel.utils;
...
import com.alphay.boot.common.excel.convert.ExcelBigNumberConvert;
import com.alphay.boot.common.excel.core.*;
import com.alphay.boot.common.excel.handler.DataWriteHandler;
...
public class ExcelUtil {
   ...
   * @return 转换后集合
   */
  public static <T> ExcelResult<T> importExcel(
      InputStream is, Class<T> clazz, ExcelListener<T> listener) {
    FastExcel.read(is, clazz, listener).sheet().doRead();
    return listener.getExcelResult();
  }
  ...
  public static <T> void exportExcel(
      List<T> list, String sheetName, Class<T> clazz, HttpServletResponse response) {
    try {
   ...
   */
  public static <T> void exportExcel(
      List<T> list, String sheetName, Class<T> clazz, OutputStream os) {
    exportExcel(list, sheetName, clazz, false, os, null);
  }
  ...
  public static <T> void exportTemplate(
      List<T> data, String filename, String templatePath, HttpServletResponse response) {
    try {
   ...
   */
  public static void exportTemplateMultiList(
      Map<String, Object> data, String templatePath, OutputStream os) {
    ClassPathResource templateResource = new ClassPathResource(templatePath);
    ExcelWriter excelWriter =
        FastExcel.write(os)
            .withTemplate(templateResource.getStream())
   ...
   */
  public static void exportTemplateMultiSheet(
      List<Map<String, Object>> data, String templatePath, OutputStream os) {
    ClassPathResource templateResource = new ClassPathResource(templatePath);
    ExcelWriter excelWriter =
        FastExcel.write(os)
            .withTemplate(templateResource.getStream())
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      for (Map.Entry<String, Object> map : data.get(i).entrySet()) {
        ...
        if (map.getValue() instanceof Collection) {
          // 多表导出必须使用 FillWrapper
          excelWriter.fill(
              new FillWrapper(map.getKey(), (Collection<?>) map.getValue()),
              fillConfig,
              writeSheet);
        } else {

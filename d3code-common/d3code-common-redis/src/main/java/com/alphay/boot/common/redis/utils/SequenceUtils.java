package com.alphay.boot.common.redis.utils;
...
public class SequenceUtils {
   ...
   * @return ID生成器
   */
  private static RIdGenerator getIdGenerator(
  ...
  public static String nextIdDate(String prefix) {
    ...
    String prefixKey =
        StringUtils.format(
            "{}{}",
            StringUtils.blankToDefault(prefix, ""),
            DateUtil.format(DateUtil.date(), DatePattern.PURE_DATE_FORMATTER));
    // 获取下一个id
    long nextId =
        getIdGenerator(prefixKey, DEFAULT_EXPIRE_TIME_DAY, DEFAULT_INIT_VALUE, DEFAULT_STEP_VALUE)
            .nextId();
    // 返回完整id
  ...
  public static String nextIdDateTime(String prefix) {
    ...
    long nextId =
        getIdGenerator(
                prefixKey, DEFAULT_EXPIRE_TIME_MINUTE, DEFAULT_INIT_VALUE, DEFAULT_STEP_VALUE)
            .nextId();
    // 返回完整id
    return StringUtils.format("{}{}", prefixKey, nextId);
  }
